...
const OrderStatusBadge = ({ status }: { status: OrderStatusEnum }) => {
  const statusConfig = {
    pending: {
      ...
      icon: ClockIcon,
      className:
        "bg-gradient-to-r from-amber-400 to-orange-500 text-white animate-pulse shadow-lg shadow-amber-200/50 ",
...
export const OrderSheetHeader = ({ status }: { status: OrderStatusEnum }) => {
  return (
    <SheetHeader className="border-b bg-muted/30 px-4 min-h-[56px]">
